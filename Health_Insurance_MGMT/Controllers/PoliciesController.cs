...
namespace Health_Insurance_MGMT.Controllers
{
    public class PoliciesController : Controller
    {
        ...
        public async Task<IActionResult> CreatePolicy(PolicyCreationViewModel model)
        {
                    ...
                    var allowedMimeTypes = new List<string>
            {
                ...
                "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    ...
                    {
                        // If the file is not a PDF or Word document, add an error message to the ModelState
                        ModelState.AddModelError("PolicyTermsAndConditions", "Invalid file type. Only PDF and Word documents are allowed.");

                        // Re-populate InsuranceCompanies to correct the selection
                        model.InsuranceCompanies = _unitofWork.InsuranceCompanyRepository.GetAll()
                            .Select(i => new SelectListItem
                            {
                                Value = i.Ins_Id.ToString(),
                    ...
                    {
                        await model.PolicyTermsAndConditions.CopyToAsync(fileStream);
                    }
            ...
            // If ModelState is invalid due to other reasons, re-populate InsuranceCompanies for the view
            model.InsuranceCompanies = _unitofWork.InsuranceCompanyRepository.GetAll()
                .Select(i => new SelectListItem
                {
                    Value = i.Ins_Id.ToString(),
